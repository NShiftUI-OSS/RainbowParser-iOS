struct RainbowLexer {
    ...
    mutating func scanTokens() throws(RainbowParseError) -> [RainbowToken] {
        ...
        while !scanner.isAtEnd {
            ...
            case ".":
                if let next = scanner.peek(), next.isRainbowIdentifierStart {
                    tokens.append(scanDotIdentifier(start: start))
                } else {
    ...
    private mutating func scanDotIdentifier(start: RainbowSourceLocation) -> RainbowToken {
        ...
        return makeToken(.dotIdentifier(text), start: start)
    }
